...
1. 집 Wi-Fi 연결, 개발자 옵션에서 **무선 디버깅**과 **USB 디버깅** 토글을 켠다.
   USB 디버깅은 **처음 한 번만** 손으로 켠다 — 서버가 뜨는 순간 앱이 `WRITE_SECURE_SETTINGS` 를 스스로 받아 두고,
   그 뒤로는 토글이 꺼져 있으면 위젯/일괄 켜기가 먼저 켠다(로그 `USB 디버깅: 꺼져 있어서 켰음`).
...
4. **재부팅하면** TCP 모드가 풀린다(`persist.adb.tcp.port` 는 삼성이 거부). 그때는 **Wi-Fi 만 켜고 위젯을 켠다** — 앱이 무선
