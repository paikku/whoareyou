...
📵 버튼은 폰 화면만 끈다(`/api/screen`; 전원 버튼은 가상 디스플레이까지 재우므로 쓰지 않는다 — 잠든 폰은 📵가 깨워서 패널만 끈다).
차가 붙어 있는 동안 서버가 화면 시간 초과를 12시간으로 밀어 폰이 저절로 잠들지 않게 한다(`/api/status.keptAwake`). 차에서 `/diag`를 열면 브라우저 환경·API
