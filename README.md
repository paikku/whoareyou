...
배열에 항목 하나 더하기**가 된다. 지금 넷이다 — 하드웨어 디코더(WebCodecs, https 전용) · 소프트 디코더
(WASM h264bsd) · `<video>`(MSE) · 낱장 그림(MJPEG). 자동으로 가장 좋은 것을 고르되 **화질 시트에서 사람이
직접 고를 수 있고**(차도 폰도 세대가 다르다) 그 선택은 그 차에 남는다. 고른 것이 나중에 못 쓰게 되면
조용히 자동으로 되돌아간다 — 검은 화면으로 알게 되는 것보다 낫다.
