...
링크와 IDR 되먹임이었다, docs/car-tests/model-y §9). 그래도 못 따라오면 그때 한 단계 자동으로 내린다(비트가 먼저, 화소는 나중). 차가 부탁한 키프레임(IDR)이 1080p 에서 575KB 까지
...
2026-09-18 리포트 재검토의 결론은 "병목은 인코더 속도가 아니라 IDR 크기 × TCP × 차의 버리기 규칙" 이었고, 코덱
교체는 그 뒤의 일이다. 인코더 손잡이 전부는 docs/agent-runbook.md "화질 · 링크".
