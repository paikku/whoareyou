...
사다리는 **차가 어떤 디코더로 열었는지에 따라 갈린다**: 평문(WASM h264bsd)은 1080p30 까지, https(하드웨어
WebCodecs)는 1080p60 까지다. **실차에서 하드웨어 경로의 디코드 지연은 1.1ms 였다** — 같은 차의 WASM 경로가
11ms 였으니 10배다(2026-09-18, docs/car-tests/model-y §8). 어느 칸까지 감당하는지는 차·링크마다 다르므로
사다리를 미리 깎지 않는다. 못 따라오면 그 자리에서 한 단계 내려가고, 그 기록이 💾 리포트에 남는다. 기억된 설정이 지금 렌더러로 못 푸는 것이면 — 북마크가 https 와 http 로 둘 있으면
생기는 일이다 — 차가 스스로 한 단계 내린다(내리지 않으면 화면이 그냥 멈춘다). 같은 시트의
