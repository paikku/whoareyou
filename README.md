...
**핫스팟 켜기가 되는지는 기기에 달렸다:** AOSP 에뮬레이터는 uid 2000 의 테더링 변경을 거부한다
(`NO_CHANGE_TETHERING_PERMISSION`, docs/verification-log.md 열린 질문 0). 거부되면 앱이 그렇게 표시하고
설정 화면으로 보낸다 — 그 경우 일괄 동작은 서버·VPN 만 다룬다. One UI 에서의 결과는 아직 미확인이다.
