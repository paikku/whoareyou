...
2. 앱 **시작** → `서버 기동 확인`. (최초 1회는 "무선 디버깅 페어링"으로 알림에 코드 입력.)
   이때 앱이 adbd를 **TCP 모드**로 전환한다(랜덤 고포트, `adb tcpip`와 같은 요청). 폰에 "USB 디버깅을 허용하시겠습니까?"가
   뜨면 **항상 허용**으로 수락. 화면의 `TCP 모드:` 줄에 포트가 표시되면 성공이다 — 이후로는 **Wi-Fi도 무선 디버깅 토글도 필요 없고**,
   차 안에서 서버가 죽어도 앱이 다시 띄운다. adbd가 계속 살아 있으므로 USB 디버깅 토글도 더는 필수가 아니다(1번은 최초 설정용).
