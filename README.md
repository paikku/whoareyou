...
앱의 **"일괄 켜기 / 일괄 끄기"** 버튼과 **바탕화면 위젯(스위치)** 은 핫스팟·서버·VPN 셋을 한 번에 다룬다.
순서는 취향이 아니다: 핫스팟을 바꿀 수 있는 것은 shell 서버뿐이라(앱 uid 에는 테더링 핫스팟 API 가 없다)
끄기는 **핫스팟 → 서버 → 세션**, 켜기는 그 반대다. 핫스팟은 서버의 loopback 전용 `POST /api/hotspot`
(`TetheringManager` 리플렉션)로 바꾸고, 상태는 `/api/status.hotspot` 에 실린다 —
`known=false` 는 "꺼짐"이 아니라 **"폰이 답해 주지 않았다"** 이다.
**단, 핫스팟은 이 폰에서 앱이 못 켠다.** S26U(One UI 8)도 에뮬레이터도 uid 2000 의 테더링 변경을
`NO_CHANGE_TETHERING_PERMISSION` 으로 거부한다(2026-09-13 실측, verification-log 열린 질문 0). 앱은 한 번
거부당하면 다시 시도하지 않고 "설정 > 모바일 핫스팟에서 직접"으로 안내하며, 일괄 동작은 서버·VPN 만 다룬다 —
그래도 누를 곳이 셋에서 둘로 준다. 코드는 남겨 둔다: 허용하는 기기에서는 그대로 동작한다.
