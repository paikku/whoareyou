...
export interface RendererStats {
   ...
   * 잠깐 막혔다는 신호라 abr 이 드롭과 같이 읽는다 — 다만 한두 장은 60fps 의 평범한 떨림이고, 재동기
   * 직후의 무더기는 우리가 만든 버스트라, 문턱과 예외가 붙는다(`abr.ts` 의 `CongestionReader`).
   */
  late?: number;
  /**
   * 지금 키프레임을 기다리느라 오는 것을 버리고 있는가(참조가 끊긴 뒤의 재동기). 이 동안의 드롭은
   * 링크가 막혀서 생긴 것이 아니라 **우리가 기다리기로 한 결과**다 — abr 이 이것을 혼잡으로 읽으면
   * 인코더를 다시 세울 때마다 비트레이트가 바닥까지 내려간다(실차 #79). main.ts 가 이 동안의 표본을 건너뛴다.
   */
  waitingForKey?: boolean;
  /**
   * 적체가 한계(HARD_BACKLOG)를 넘어 재동기한 **횟수**. 드롭 프레임 수와 달리 기다리는 길이에 비례해
   * 부풀지 않아서, abr 이 읽기에 맞는 크기다. webcodecs 렌더러만 낸다.
   */
  overloads?: number;
