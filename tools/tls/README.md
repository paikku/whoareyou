...
있는 것은 폰 위에서 도는 것뿐이고, 그것이 앱과 adb 셸이다.

## PC 없이: 앱의 "인증서 받아서 심기" (2026-09-17)

인증서가 만료되는 날 운전자가 들고 있는 것은 폰뿐이다. 그래서 위의 두 줄(받기 → 심기)을 앱이 한다:
**자세히 → 인증서 받아서 심기**. 주소 하나를 받아 `https` 로 내려받고, 인증서와 키가 맞는지만 보고,
loopback 의 `POST /api/tls` 로 넘긴다. 판정은 여전히 서버가 한다(세워 본 뒤에만 저장).

- 기본 주소는 `https://local-ip.sh` — 체인(`/server.pem`)과 키(`/server.key`)를 따로 주므로 앱이 붙인다.
  공개 키 출처라 앱이 그 사실을 로그에 한 줄 남긴다.
- 우리 도메인은 **한 덩어리**(`cat cert.pem key.pem`)를 주는 https 주소를 넣는다. 주소는 기억된다.
- **https 주소만 받는다.** 개인키가 지나가는 길이라 평문은 거절한다(`CertInstall.download`).

**그리고 이 버튼은 스스로도 눌린다.** 세션이 서버 상태를 읽을 때마다 `CertInstall.autoRenew` 가 묻는다 —
공개 CA 인증서의 만료가 **14일 안**이거나 **자체서명**이면(이 차는 그 경고를 넘지 못하니 그대로 두면 하드웨어
경로가 영영 없다) 같은 주소에서 받아 심는다. 하루에 한 번만 움직이고, 실패해도 지금 인증서는 그대로다.
사용자가 할 일은 없다. "한 번으로 끝"은 없다 — 공개 인증서의 유효기간은 브라우저 규칙으로 줄어드는 중이라
(398일 → 2029년 45일) 갱신을 없앨 수는 없고, 안 보이게 할 수 있을 뿐이다.

계약은 `app/.../service/CertInstall.kt` 와 `CertInstallTest` 에 있다: 두 파일 출처와 한 덩어리 출처,
PEM 이 아닌 것은 **서버에 닿기 전에** 멈추는 것, 서버의 거부 사유를 지어내지 않고 그대로 옮기는 것,
그리고 진짜 소켓으로 loopback 에 POST 하는 것.
