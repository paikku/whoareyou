...
// 화면이 정지해 있어도 인코더는 REPEAT_PREVIOUS_FRAME_AFTER 로 계속 뱉어야 한다 —
// 이 구간이 비는 것이 2026-09-10 실차에서 멈춤을 만든 원인이었다(docs/verification-log.md).
import { strict as assert } from 'node:assert';
import { test } from 'node:test';
import { collectVideo, serverLog, status } from '../lib.mjs';

const WINDOW_MS = 8_000;

test('붙자마자 init 세그먼트와 키프레임이 오고, 그 뒤로 프레임이 끊기지 않는다', async () => {
