...
(안드로이드 설계; 📵는 패널만 끄고 폰은 깨어 있다). 그래서 **차가 붙어 있는 동안 전원 버튼은 "패널 토글"로 동작**한다: 누르면 폰이 잠들고
차가 1~2초 안에 이를 보고 `KEYCODE_WAKEUP`으로 깨운다. 가상 디스플레이는 자기 전원 그룹이라 그래도 꺼져 있으면(`/api/status.displayState` ≠ 2)
**VD를 새로 만들고 마지막 앱을 다시 띄운다**(앱은 처음부터 시작). 패널은 누르기 전 상태의 반대로: 켜져 있었으면 끄고(화면만 끄려던 것),
서버가 꺼 두고 있었으면 켠다(폰을 쓰려던 것 — 예전엔 이때 두 번 눌러야 켜졌다). 서버 로그에
`자동 복구: 폰 깨움, VD는 그대로 꺼져 있음 → 가상 디스플레이 재생성: id 7 → 8, 앱 … 다시 실행, 패널 끄기: true`.
차가 떠나면(마지막 클라이언트 끊김 15초 뒤) 패널을 켜 두어 폰이 평소처럼 동작한다.
지연: 폰이 실제로 잠든 뒤에 되살리는 구조라 전원 버튼 → 패널 반응까지 0.3~0.6초는 불가피하다(50ms 감시 + 인프로세스 KEYCODE_WAKEUP 주입 + 깨어남 확인).
켜기는 컨트롤러의 꺼짐→켜짐 절차가 필요하다: **디스플레이 0이 실제로 OFF가 된 것을 확인한 뒤** 깨우고 ON을 확인한다(`ScreenPower.lightPanel`).
PowerManager의 "잠듦"은 버튼을 누르는 순간 바뀌지만 패널·백라이트는 그보다 늦게 꺼지므로, 그 전에 깨우면 컨트롤러가 백라이트를 다시 올리지 않아 폰이 어둡게 남는다
(리포트 #19 `7 ms`, #21 `183 ms` — 둘 다 폰이 안 켜졌다). SurfaceControl NORMAL만으로는 백라이트가 0이다.
**탈출구:** 10초 안에 전원 버튼 3번 → 무조건 폰 화면을 켜고 30초 동안 자동 복구를 멈춘다(`/api/status.autoPausedMs`; 차는 📵로 복구).
**모든 잠듦이 각각 복구된다** — 끄고 곧바로 다시 켜도 두 번째 누름을 버리지 않는다(리포트 #20: 5초 게이트가 두 번째 누름을 버려 폰이 잠든 채 남고 이후 의도가 뒤집혔다).
VD가 안 깨어난 경우 Android 15+ `requestDisplayPower(vdId, on)`를 먼저 시도해 앱을 살리고, 그래도 안 되면 재생성한다 — 어느 경로였는지와 걸린 ms가 `자동 복구:` 로그 줄 끝에 남는다.
확인 항목: ① 전원 버튼 → 2~3초 안에 노트북 영상이 돌아오는가(앱 첫 화면), 폰 패널은 꺼져 있는가 ② 다시 전원 버튼 → 폰 화면이 **한 번에** 켜지고 노트북 영상도 유지되는가
③ 📵로 끄고 켜기가 여전히 되는가 ④ 노트북 페이지를 닫고 15초 뒤 폰 패널이 켜지는가.
