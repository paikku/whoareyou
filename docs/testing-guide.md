...
`/api/status.screenOn`으로도 확인. **전원 버튼은 누르지 않는다** — 폰이 잠들면 가상 디스플레이도 같이 잠들어 차 화면이 마지막 프레임에서 멈춘다
(안드로이드 설계; 📵는 패널만 끄고 폰은 깨어 있다). 눌렀다면 **차가 붙어 있는 동안은 서버가 1~2초 안에 스스로 복구**한다:
`KEYCODE_WAKEUP`으로 폰을 깨우고, 가상 디스플레이는 자기 전원 그룹이라 그래도 꺼져 있으면(`/api/status.displayState` ≠ 2) **VD를 새로 만들고
마지막 앱을 다시 띄운 뒤**(앱은 처음부터 시작) 패널만 다시 끈다. 서버 로그에 `자동 복구: 폰 깨움, VD는 그대로 꺼져 있음 → 가상 디스플레이 재생성: id 7 → 8, 앱 … 다시 실행`.
**1분 안에 다시 전원 버튼을 누르면 "폰을 쓰려는 것"으로 보고 그대로 둔다** — 차 상태줄에 `😴폰 잠듦`, 📵를 누르면 같은 복구를 한다.
확인 항목: ① 전원 버튼 → 2~3초 안에 노트북 영상이 돌아오는가(앱 첫 화면), 폰 패널은 꺼져 있는가 ② 1분 안에 두 번째 전원 버튼 → 폰이 잠든 채 유지되고 `😴`가 뜨는가 ③ 그 상태에서 📵 → 복구되는가.
그리고 차가 붙어 있는 동안은 서버가 `screen_off_timeout`을 12시간으로 밀어 충전기 없이도 폰이 저절로 잠들지 않아야 한다
(`/api/status.keptAwake`, 마지막 클라이언트가 떠나고 15초 뒤 원래 값으로 복구, 서버 로그에 `screen_off_timeout = …`).
