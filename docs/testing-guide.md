...
4b. **본 화면은 https 로 연다.** 평문으로 열면 차는 WASM 디코더로 풀고, 하드웨어 디코더에는 닿지
   ...
   **"하드웨어 디코더"** 를 누르면 같은 곳으로 데려다준다(폰이 공개 CA 인증서를 들고 있을 때만;
   ...
   - **1080p60 을 다시 눌러 본다 (2026-09-18 이후 빌드).** 이번에는 사다리보다 **abr** 이 먼저 움직여야 한다:
     ...
     rtt 가 뛰는 표본에서 `keyBytes` 가 크면 IDR 버스트가 범인이고, 그때는 먼저 화질 시트의 **"키프레임 크기 상한"**
     을 올린다. 그래도 크면 **"키프레임 대신 인트라 리프레시"**(실험, 기본 꺼짐, 이번 세션만 산다)를 켜고 1 분 더
     둔 뒤 💾 로 세션을 하나 더 남긴다 — 껐을 때와 켰을 때의 `keys`·`keyBytes`·`kbps` 를 견주는 것이 이 손잡이의
     유일한 판정이다(agent-runbook §3 "화질 · 링크").
   ...
   - **먼저 요약 줄에 `⚠페이지 build=` 가 없는지 본다 (열린 질문 20).** APK 를 새로 깔았는데 차의 탭이 열린
     ...
     새로고침하고 시작한다.
   - ✅ **여기까지는 2026-09-19 에 통과했다** (빌드 `d8fd1d2`, report #84, car-tests §16): 1080p60 4.3 분에
     드롭 0 · 늦음 0 · 적체 초과 0 · `decode stall` 0, 실제로 받은 비트 9~10.4 Mbps. 아래는 그 재현 절차다.
