...
4b. **본 화면은 https 로 연다.** 평문으로 열면 차는 WASM 디코더로 풀고, 하드웨어 디코더에는 닿지
   ...
   **"하드웨어 디코더"** 를 누르면 같은 곳으로 데려다준다(폰이 공개 CA 인증서를 들고 있을 때만;
   ...
   - **`keyBytes` 를 먼저 본다 (빌드 `e0ee6d2` 이후, 열린 질문 13).** I 프레임 QP 상한(28)이 먹었으면 1080p 에서
     100 KB 안팎이어야 한다. 여전히 250 KB 를 넘으면 벤더가 그 키를 무시한 것이고, `/api/status.encoderProfile`
     에 "I-QP 상한 거부됨" 이 있는지 본다. 키프레임 직후 화질 펄스가 눈에 띄면 `qp_i_max=32` 쪽으로.
