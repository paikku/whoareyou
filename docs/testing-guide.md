...
4b. **본 화면도 https 로 한 번.** 평문으로 열면 차는 WASM 디코더로 풀고, 하드웨어 디코더에는 닿지
   못한다(`VideoDecoder` 가 secure context 전용이라 그렇다). 화질 시트의 **"하드웨어 디코더로 열기"**
   를 누르거나 앱 화면의 `URL(빠름·하드웨어 디코더)` 를 그대로 친다. 확인할 것:
