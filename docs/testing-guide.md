...
| **핫스팟 일괄 켜기·끄기 / 바탕화면 위젯** | 단위 테스트만: `HotspotTest`(loopback 규칙, "모름"과 "꺼짐"을 가르는 것), `BulkControlTest`(끄기 순서를 가짜 서버로), `CarCastWidgetTest`(스위치 상태 규칙) | `08-hotspot.test.mjs`: 엔드포인트가 있고 늘 JSON 으로 답하는지, `/api/status.hotspot` 이 실리는지, **위젯 provider 가 `dumpsys appwidget` 에 등록됐는지**, 핫스팟 요청이 서버를 죽이지 않는지. **진짜 AP 는 없다** | **핵심.** 핫스팟이 실제로 켜지고 꺼지는지, 통신사 entitlement 우회가 필요한지, 그리고 **켠 뒤 서버가 살아남는지**(TCP 모드 없으면 §3.5 대로 죽는다). 홈에 위젯을 실제로 얹어 보기 | 차를 핫스팟에 붙인 채 위젯으로 끄고 다시 켜기 |
...
빌드 `TaskList`(이 커밋)부터 서버는 앱을 띄우기 전에 `am stack list`로 task 위치를 보고, **다른 디스플레이에 있으면 강제 종료 후 새로 띄운다**
(`/api/app?name=…&restart=auto|always|never`, 기본 `auto`). 확인은 노트북으로 하되 실차와 같은 순서로:
1. **폰 → 차:** 폰 화면에서 유튜브를 열어 영상을 하나 재생해 둔 채, 노트북 ▶에 `com.google.android.youtube`.
   기대: 응답 `action:"restarted", fromDisplay:0`, 노트북 상단에 "폰에서 쓰던 앱을 종료하고 차 화면에 새로 띄움", 폰에는 유튜브가 **닫혀 있고**
   노트북에는 유튜브 첫 화면. (`restart=never`를 붙이면 예전 동작: `action:"moved"`, 폰에서 사라지고 노트북에 재생 중이던 화면이 그대로 나타나야 한다 — 이것이 "그대로 가져와지는지"의 답.)
2. **차 → 폰:** 노트북에서 유튜브가 도는 상태에서 폰 런처의 유튜브 아이콘을 누른다.
   ...
   `/api/status`에 `appDisplay:0, appOnPhone:true`. 서버는 스스로 되찾지 **않는다**(되찾으면 사용자와 앱을 뺏는 싸움이 된다). ▶를 다시 누르면 1번과 같다.
3. **재실행 중 아무것도 없을 때:** 앱을 완전히 닫은 상태에서 ▶ → `action:"started", fromDisplay:null`.
