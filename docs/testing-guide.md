...
(안드로이드 설계; 📵는 패널만 끄고 폰은 깨어 있다). 눌렀다면: 차 상태줄에 `😴폰 잠듦`이 뜨고(`/api/status.asleep`), 📵를 누르면 서버가
폰을 깨운 뒤(`KEYCODE_WAKEUP`) 패널만 끈다 — 이것도 확인 항목. 그리고 차가 붙어 있는 동안은 서버가 `screen_off_timeout`을 12시간으로 밀어
충전기 없이도 폰이 저절로 잠들지 않아야 한다(`/api/status.keptAwake`, 마지막 클라이언트가 떠나고 15초 뒤 원래 값으로 복구, 서버 로그에 `screen_off_timeout = …`).
