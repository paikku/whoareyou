...
3. 상단에 `저장됨 #n`이 뜰 때까지 기다린다(약 40초: WS 20회 + 영상 5초 + 사설 주소 확인).
   ...
   **한 번 더, https 로.** 같은 페이지를 `https://100.99.9.9:3443/diag.html` 에서 연다. 자체서명
   인증서라 "연결이 비공개가 아닙니다"가 뜨는데 **고급 → 계속**을 누른다(그 경고가 정상이다).
   거기서만 답이 나오는 질문이 하나 있다: **이 차에 WebCodecs(하드웨어 디코더)가 있는가.**
   `VideoDecoder` 는 secure context 에서만 존재하므로 평문에서 본 `WebCodecs X` 는 답이 아니다.
   - 경고가 안 뜨고 그냥 안 열리면 → 차 브라우저가 생 IP https 를 안 여는 것. 거기서 멈추고 기록한다.
