...
4b. **본 화면은 https 로 연다.** 평문으로 열면 차는 WASM 디코더로 풀고, 하드웨어 디코더에는 닿지
   ...
   **"하드웨어 디코더"** 를 누르면 같은 곳으로 데려다준다(폰이 공개 CA 인증서를 들고 있을 때만;
   ...
   - **https 의 `/diag` 를 한 번 더 저장한다 (열린 질문 16·17).** "하드웨어 디코더" 표에 HEVC·AV1·VP9 줄이,
     "WebRTC" 표에 루프백 결과(협상 코덱·`decoderImplementation`)가 실린다. 코덱을 바꾸거나 전송을 바꿀 수
     있는지의 차 쪽 절반이 이것이고, 폰 쪽 절반은 `/api/status.encoders` 다.
