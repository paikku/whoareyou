...
| **오디오 (M6)** | `audio.spec.ts`: 테스트 톤이 `/ws/audio`로 와서 `<audio>`가 영상과 ±150ms 안에서 진행, 🔊 토글, 오디오 없는 폰에서 영상 무영향, `/diag` 오디오 프로브 | `/api/status.audio`가 `source=display`·`frames` 증가, 폰 스피커 무음, 노트북에서 소리·A/V 동기 | 차 스피커로 재생, 첫 터치 후 소리(자동재생 정책), 동기 |
...
| 소리(M6): 차 스피커 출력, 첫 터치 후 소리가 나는지(오디오 자동재생 정책), 영상과의 동기 체감 | 2026-09-10 구현. 노트북은 디코드·동기 수치까지만 |
...
- **소리(M6):** 폰에서 유튜브 등을 틀고 노트북 본 화면(`http://100.99.9.9:3333/`)을 첫 터치한 뒤 노트북에서 소리가 나는지, 폰 스피커는 무음인지,
  하단 상태줄에 `♪err`가 없는지 본다. 앱 화면/`/api/status`의 `audio` 항목이 `source=display`이고 `frames`가 늘어야 한다.
  `source=failed`면 그 `error`를 적어 둔다(차에서는 소리 없이 영상만 나온다).

**차에서:**
1. 폰 앱이 실행 중(`tun: UP`, 서버 응답 중)인지 보고, 차를 폰 핫스팟에 연결.
2. 차 브라우저에서 `http://100.99.9.9:3333/diag` → 열리면 가정 2 통과.
3. 상단에 `저장됨 #n`이 뜰 때까지 기다린다(약 50초: WS 20회 + 영상 5초 + 오디오 5초 + 사설 주소 확인).
   페이지가 UA·viewport·DPR·API 표·WS 성공률·fps·lag·**오디오 패킷/진행**·**사설 주소 차단 여부**를 모두 측정해 폰에 POST한다.
   `저장 실패`가 뜨면 그때만 화면을 사진으로 찍는다.
4. `http://100.99.9.9:3333/` 에서 첫 터치 후 영상 재생, 전체화면 버튼, 후진 기어 전환 후 복구.
   **소리:** 폰에서 유튜브(▶ 버튼)를 틀고 첫 터치 뒤 차 스피커에서 소리가 나는지, 입과 소리가 맞는지(어긋나면 앞/뒤 어느 쪽인지),
   🔊 버튼으로 껐다 켜지는지. 안 나면 브라우저 볼륨/차 미디어 볼륨을 먼저 보고, 💾로 세션 리포트를 남긴다(`audio ...f sync ...ms paused/err`가 원인을 말해 준다).
   오디오가 켜지면 영상이 라이브 엣지에서 180ms 뒤(기존 50ms)에서 돌아 터치가 조금 늦어진다 — 체감이 나쁘면 `?audio=off`와 비교해 적어 둔다.
