...
> 2026-09-05: 첫 폰 시도에서 재부팅 뒤 무선 디버깅 connect 포트가 계속 거부돼 앱이 아예 붙지 못했다(verification-log §3.7).
...
| 2 | `persist.adb.tcp.port` — 앱이 자동으로 시도하므로 **로그만 확인** 후 재부팅 → Wi-Fi 없이 붙는지 | 5분 | 콜드 부팅 Wi-Fi 제거 (§3) |
