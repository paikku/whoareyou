...
> 2026-09-04 이후 상태: 이 제안의 가정들이 어디까지 확인됐는지는 [verification-log.md](verification-log.md), 확정된 계획은 [dev-plan.md](dev-plan.md)를 본다. 특히 가정 1은 "서버 소켓이 shell uid일 때만" 성립하는 것으로 실측돼 서버가 shell 프로세스로 옮겨갔다.
