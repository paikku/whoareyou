...
| M7-c | 전원 버튼 토글 (빌드 `0470765`, 2026-09-10 20:43, 노트북 리포트 #23·#24) | ✅ 켜기 `패널 OFF 확인 후 깨움 → 패널 ON` 500~740ms, 끄기 `폰 깨움, 패널 ON 확인, 패널 끄기` 350~610ms, VD는 매번 스스로 돌아옴(재생성 없음). 사용자 확인 "꺼지고 켜지는 거 잘 됨". 이전 리포트 #19~#22는 옛 서버(빌드 불일치)에서 찍힌 것 |
| — | "가끔 끊김" (리포트 #24 `decode stall … buf=[]` 2회) | 원인: 클라이언트 trim이 재생 위치−8초 기준으로 지우다 정지 화면(8초 이상 새 프레임 없음)에서 마지막 키프레임까지 삭제 → 이후 P-프레임은 참조가 없어 MSE가 버림 → 인코더 키프레임 간격은 프레임 수 기준(60fps 선언에 2초 = 120프레임)이라 정지 화면에선 수십 초 → 감시기 2초 뒤 재접속. 수정: trim 상한을 마지막 키프레임 pts로. e2e `static-gap.spec.ts`(가짜 폰 `--pause-every/--pause-ms`)로 재현·검증 |
