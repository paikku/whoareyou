...
| 3 | shell 권한으로 띄운 scrcpy 서버 포크가 갤럭시에서 VD 생성 + 타 앱 실행 + 터치 주입이 된다 | ⏳ M0 미실시. 단 "앱 자신의 APK를 `app_process`로 shell uid에서 실행"은 ✅, **앱이 내장 ADB로 직접 띄우는 것도 ✅** | B | §3.3, §3.5 |
...
| — | 분리 실행(`daemon=true`) 서버가 adb 스트림·Wi-Fi·무선 디버깅 종료·화면 OFF 후에도 유지된다 | ✅ 앱이 띄운 서버가 핫스팟 전환·화면 OFF 후에도 응답 (2026-09-05). 하룻밤·재부팅은 ⏳ | B | §3.5 |
