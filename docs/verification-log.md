...
| — | **인코더의 11~14 ms 는 화소가 아니라 파이프라인이다** → `KEY_OPERATING_RATE` + 퀄컴 저지연 키로 내려간다 | ⏳ **가설의 근거는 있다** (리포트 #68~#73 재검토: 720p30 14 ms > 900p60 11.5 ms, 720p 와 900p 가 같음 — car-tests/model-y §9). 효과는 B 에서 `timing.encodeMs` 로 잰다. 거부하는 인코더는 벤더 기본값으로 물러난다(A+ 에서 `09-encoder` 가 그림이 계속 나오는지 본다) | B | `H264Encoder.format` |
...
| — | **링크가 막히면 차가 비트레이트를 먼저 내린다** (`abr.ts`; 사다리는 바닥 뒤에만) | ✅ A `quality.spec` "링크가 막히면…" (기준 rtt → 두 표본 연속 두 배 → 25% 컷, 드롭은 즉시, 5 초 간격, 20 초 뒤 되올림, 프리셋 바꾸면 재시작). 실차에서 #70 의 1080p60 을 다시 눌러 `abr ↓` 이벤트와 드롭 수를 본다 | A → C | car-tests/model-y §9 |
