...
| — | **차가 부탁한 IDR 은 비트레이트 목표와 무관하게 커서(1080p 550~575 KB) 그것이 멈춤의 증폭기다** → `KEY_VIDEO_QP_I_MAX`(기본 28)로 IDR 크기를 잡는다 | ⏳ 근거는 실차 report #76(car-tests/model-y §11): abr 이 12000 → 5000k 로 내리는 동안에도 요청 IDR 은 575 KB, rtt 는 평평. 효과는 다음 세션의 perf `keyBytes` 로 본다(100 KB 안팎 기대). A+ `09-encoder` 가 손잡이의 왕복을 확인 | C | `H264Encoder`, `Server.DEFAULT_QP_I_MAX` |
...
12. **하드웨어 경로의 끝에서 끝까지 지연 (C):** #70 은 `latencyProbe: null` 이라 글래스 투 글래스 값이
   ...
   한 번 누르면 된다(30초).
13. **I 프레임 QP 경계가 실차에서 IDR 을 잡는가, 그리고 그 뒤에도 1080p60 이 무너지는가 (C): ✅ 앞의 절반은 답 나옴
...
14. **하드웨어 경로의 끝에서 끝까지 지연이 인코더 5 ms 만큼 줄었는가 (C):** 12 와 같은 측정. 720p30 43 ms 가
   38 안팎이어야 한다.
15. **차 쪽 되먹임을 끊은 것이 실차에서 보이는가 (C): ⚠️ 절반만 (2026-09-19, #79, car-tests §14) — 밀린 프레임은
...
20. **새 APK 를 깐 뒤 차의 페이지가 새 것으로 바뀌는가 (C):** #83 은 폰만 새 빌드였고 차의 탭은 39 분 전에
...
19. **abr 이 재동기를 혼잡으로 읽던 되먹임이 끊겼는가 (C):** #79 에서 abr 은 2.5 분에 **자르기 12·올리기 0**,
