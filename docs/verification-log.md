...
| — | **앱이 USB 디버깅 토글을 스스로 켤 수 있다** (`pm grant … WRITE_SECURE_SETTINGS` 를 shell 링크가 뜰 때 앱이 직접 실행 → 이후 shell 없이 `Settings.Global.adb_enabled=1`) | ⏳ **구현됨 (2026-09-14), 폰 미검증.** 위젯/일괄 켜기의 1/3 단계가 이것이다. 배경: Wi-Fi 가 끊길 때 살아남는 조건이 USB 디버깅이고(§3.5), 그 토글을 매번 손으로 켜고 있었다. 확인할 것: ① 서버가 한 번 뜬 뒤 로그에 `WRITE_SECURE_SETTINGS 부여됨` ② 토글을 끄고 Wi-Fi 없이 위젯 ON → 로그 `USB 디버깅: 꺼져 있어서 켰음` → TCP 모드 포트 재개방 → 서버 기동 ③ One UI 가 값을 되돌리지 않는지(`REFUSED` 로그) ④ 재부팅 뒤에도 grant 가 남는지 | A+ → B | `UsbDebugging.kt`, `BulkControl.allOn` |
