...
| — | **링크가 막히면 차가 비트레이트를 먼저 내린다** (`abr.ts`; 사다리는 바닥 뒤에만) | ✅ A `quality.spec` "링크가 막히면…" (기준 rtt → 두 표본 연속 두 배 → 25% 컷, 드롭은 즉시, 5 초 간격, 20 초 뒤 되올림, 프리셋 바꾸면 재시작). ⏳ **실차는 아직**: 첫 세션(report #75)은 "자동" 토글에 묶여 있어 한 번도 안 돌았다(`abr.active false`) → 토글에서 떼어냈다(§10). 다음 세션에서 900p60·1080p60 의 `abr ↓` 이벤트를 본다 | A → C | car-tests/model-y §9·§10 |
