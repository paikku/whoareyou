...
| — | **앱이 USB 디버깅 토글을 스스로 켤 수 있다** (`pm grant … WRITE_SECURE_SETTINGS` 를 shell 링크가 뜰 때 앱이 직접 실행 → 이후 shell 없이 `Settings.Global.adb_enabled=1`) | ✅ **확인됨 (2026-09-14 20:56, S26U/One UI 8, Wi-Fi 없음, 빌드 `f8e7e89`)**: 토글 꺼진 채 위젯 ON → `USB 디버깅: 꺼져 있어서 켰음` → 5초 뒤 `TCP 모드 포트 31432: 열려 있음`(adbd 가 `service.adb.tcp.port` 를 읽어 재개방) → `adb 접속(TCP 모드): uid=2000` → 서버 기동. **손댄 것 없이 위젯 하나로 끝난 첫 사례.** grant 가 이미 있었으므로 ① 도 통과. 남은 것: ④ 재부팅 뒤에도 grant 가 남는지(설계상 남지만 미실측). 그 5초 동안 링크 루프가 "Wi-Fi 없음" 을 찍던 것은 `BulkControl.awaitTcpPort` 로 막음 | B | `UsbDebugging.kt`, `BulkControl.allOn` |
| — | **앱이 무선 디버깅 토글도 스스로 켤 수 있다** (`adb_wifi_enabled=1`, 같은 grant) — 재부팅 뒤 TCP 포트가 없을 때 남는 유일한 손작업이 이것 | ⏳ **구현됨 (2026-09-14), 미실측.** `ShellServerLink` 루프가 Wi-Fi 연결 중 + 토글 꺼짐을 보면 한 번 써 본다. 확인: 재부팅 → Wi-Fi 연결 → 위젯 ON → 로그 `무선 디버깅: 꺼져 있어서 켰음` 뒤에 `_adb-tls-connect` 포트 발견·접속이 이어지는가. AdbService 가 부팅 때 이 값을 0 으로 되돌리는 코드가 있어 앱의 쓰기도 되돌릴 수 있다 — 그러면 `REFUSED` 가 찍히고 잃는 건 없다 | B | `UsbDebugging.ensureWirelessOn` |
