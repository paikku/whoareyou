...
| — | **인코더의 11~14 ms 는 화소가 아니라 파이프라인이다** → `KEY_OPERATING_RATE` + 퀄컴 저지연 키로 내려간다 | ✅ **실차 확인 (2026-09-18, 빌드 `2b42626`, report #74·#75)**: `c2.qti.avc.encoder` 가 두 키를 받았고(`encoderProfile` 에 "거부됨" 없음) `encodeMs` p50 이 **720p30 14 → 8.6, 900p30 14 → 9.4** (p90 11~13). 근거는 리포트 #68~#73 재검토(car-tests/model-y §9), 결과는 §10 | C | `H264Encoder.format` |
| — | **encoder.conf 가 프로파일을 기억해 접속마다 인코더를 두 번 세우지 않는다** | ✅ **실차 (report #75)**: `0.1s init segment avc1.640020` 하나뿐, 두 번째 init 없음 | C | `EncoderSettings` |
| — | **차가 부탁한 IDR 이 멈춤의 증폭기다** → I 프레임 QP **하한**(`KEY_VIDEO_QP_I_MIN`, 기본 28)이 그 크기를 잡는다 | ✅ **하한은 먹는다**(실차 #79·#83·#84): 1080p60 의 요청 IDR 이 하한 28 에서 86~143 KB, 44 에서 15~35 KB, 하한 없던 #76 은 260~575 KB. ⚠️ 다만 **증폭기는 IDR 이 아니었다** — #84 는 하한 28 구간에서도 드롭 0 이다. 진짜 고리는 재동기 드롭을 혼잡으로 읽던 abr 이었다(§14·§16). 키 이름이 `qp_i_max` 였던 빌드(`e0ee6d2`~`966a320`)는 방향이 거꾸로였다 | C | `H264Encoder.format`, car-tests/model-y §13·§14·§16 |
| — | **1080p60 이 실차에서 끝까지 돈다** (하드웨어 디코더 + abr 이 재동기를 혼잡으로 안 읽을 때) | ✅ **실차 확인 (2026-09-19, 빌드 `d8fd1d2`, report #84)**: 4.3 분 동안 **드롭 0 · 늦음 0 · 적체 초과 0 · `decode stall` 0**, 키프레임 요청 4(전부 인코더 재시작 직후), abr `↓1↑0`(그 한 번은 진짜 rtt 101 ms), 실제로 받은 비트 **9~10.4 Mbps**, 폰 `encodeMs` p50 8.4 ms. 패킷 11,814 중 11,810 장 디코드. 같은 차·같은 링크의 #79 는 드롭 643·비트↓12↑0·바닥 4.8 Mbps 였다 | C | `abr.ts` `CongestionReader`, car-tests/model-y §16 |
| — | **비트레이트 라이브 변경이 실차에서 된다** | ✅ **실차 (report #76)**: `abr ↓ 9000k → 6700k → 5000k`, 폰 `bitrateChanges 5`, `encoderRestarts` 는 사람이 고른 3 뿐 | C | `DisplayVideoSource.reconfigure` |
| — | **송신 버퍼 64 KB 로 폰이 링크 압력을 본다** | ✅ **실차 (report #74·#75)**: `videoStaleDropped 2` — 이전 세션은 전부 0 이었다. 900p60 의 드롭 창과 같은 순간 | C | `StreamSession.VIDEO_SEND_BUFFER_BYTES` |
| — | **비트레이트만 바꾸면 인코더를 다시 세우지 않는다** (`PARAMETER_KEY_VIDEO_BITRATE`, 응답 `rebuilt:false`) | ⏳ A+ `09-encoder` "비트레이트만 바꾸면…" 가 재빌드 0·`bitrateChanges` +1 을 확인한다. 벤더가 파라미터를 받고도 무시할 가능성은 실차 perf 의 `kbps` 가 답한다 | A+ → C | `DisplayVideoSource.reconfigure` |
| — | **링크가 막히면 차가 비트레이트를 먼저 내린다** (`abr.ts`; 사다리는 바닥 뒤에만) | ✅ A `quality.spec` "링크가 막히면…" (기준 rtt → 두 표본 연속 두 배 → 25% 컷, 드롭은 즉시, 5 초 간격, 20 초 뒤 되올림, 프리셋 바꾸면 재시작). ✅ **실차 (report #76, §11)**: 드롭이 보이자 5 초 간격으로 12000 → 9000 → 6700 → 5000k, 재빌드 없이. **다만 그 세션의 멈춤은 링크가 아니라 IDR 크기였으므로 내려도 낫지 않았다** — abr 은 rtt 가 오르는 종류의 문제(#70)를 위한 층이고, IDR 은 위 행의 QP 상한이 맡는다 | A → C | car-tests/model-y §9·§10 |
| — | **영상 소켓의 커널 송신 버퍼가 폰의 큐를 눈멀게 한다** → 64 KB 로 제한 | ⏳ #70 에서 `queued 0` 인 채 rtt 69 ms 가 그 증거. 제한 뒤에는 같은 상황에서 `videoStaleDropped` 가 0 이 아니어야 한다 | C | `StreamSession.VIDEO_SEND_BUFFER_BYTES` |
...
11. **1080p60 이 실제로 이 차에서 도는가 (C): 답 나옴 — ❌ 이 차·이 링크에서는 못 버틴다 (2026-09-18,
   ...
   - ~~**남은 미지수: 링크인가 디코더인가.**~~ **답 나옴 (2026-09-18, 리포트 재검토 §9 + report #76 §11): 둘 다
     아니고 키프레임 요청에 답하는 IDR 의 크기다.** #70 의 rtt 튐은 링크가 눌린 것이 맞지만 그 앞에 IDR 요청
     14 회가 있었고, #76 에서는 rtt 가 평평한 채 요청 IDR 이 550~575 KB 로 나가며 같은 모양으로 무너졌다.
     abr 이 목표를 12000 → 5000k 로 내려도 IDR 크기는 그대로였다. 폰이 `queued 0` 이었던 것은 커널 송신
     버퍼 뒤의 맹점(→ 64 KB 로 제한). 이제 I 프레임 QP 상한(28)이 IDR 크기를 잡는다 — 실차 확인은 열린 질문 13.
...
12. **하드웨어 경로의 끝에서 끝까지 지연 (C):** #70 은 `latencyProbe: null` 이라 글래스 투 글래스 값이
   ...
   한 번 누르면 된다(30초).
13. **I 프레임 QP 경계가 실차에서 IDR 을 잡는가, 그리고 그 뒤에도 1080p60 이 무너지는가 (C): ✅ 둘 다 답 나옴 —
   **1080p60 은 더 이상 안 무너진다**(2026-09-19, #84, §16: 4.3 분 드롭 0, 9~10.4 Mbps). 하한 44 는 요청 IDR 을
   15~35 KB 로 잡지만 하한 28(58~143 KB)에서도 드롭이 0 이라, 되먹임이 끊긴 지금 IDR 크기는 병목이 아니다 —
   기본값은 28 로 둔다. (앞의 절반:) ✅
   (2026-09-19, 실차 report #79, car-tests §14) — 실제 화면(유튜브)에서 1080p60 의 요청 IDR 은 상한 28 에서 **108~111 KB**,
   32 에서 78~79 KB, 44 에서 29 KB, 720p60 44 에서 12.6~13.1 KB 다. 하한 없던 #76 의 260~575 KB 에 견주면 **기본값 28 만으로
   5 배**가 줄었고, 계단은 28~44 내내 살아 있다. 화질 펄스는 눈으로 판정한 기록이 없어 기본값은 28 에 그대로 둔다.
   뒤의 절반(그래도 1080p60 이 무너지는가)은 이 세션으로는 못 가린다 — abr 이 20 초 만에 4.8 Mbps 로 가둬 버렸다(질문 19).
   (그 전 기록:) ⚠️ 방향이 거꾸로였다
   (2026-09-19, 폰 벤치, car-tests §13) — `qp_i_max=28` 은 화질 하한이라 요청 IDR 이 104 KB(상한 없음 49 KB)로 **두 배**
   커졌다. 크기 상한은 `qp_i_min` 이고 빌드 §13 이후 기본 28·max 0 이다. 옛 encoder.conf 의 `qp_i_max=28` 은 읽을 때
   버린다. 실차 확인은 새 빌드로 다시. **벤더가 이 키를 지키는 것은 폰 벤치로 확정**(§13 세 번째 실행: gradient 에서
   `qp_i_min=40` 이 요청 IDR 48.6 → 7.0 KB); noise 에서는 36 까지 안 물렸으니 실제 화면에서 물리는 값은 차에서 화질
   시트의 "키프레임 크기 상한"(0·28·32·36·40·44)을 돌려 perf `keyBytes` 로 찾는다. 요청 직전 비트레이트 딥은 IDR 을
   못 바꾼다(바이트까지 같음) — 라이브 인코더에 넣지 않는다.** (원래 질문:)
   빌드 `e0ee6d2` 부터. 볼 것은 perf 의 `keyBytes`(§11 은 261~575 KB), 키프레임 직후 화질 펄스가 눈에 띄는지
   (띄면 `qp_i_max=32`), 그리고 그래도 무너질 때 rtt·keyBytes 가 둘 다 낮은지 — 그러면 남는 용의자는 차의
...
14. **하드웨어 경로의 끝에서 끝까지 지연이 인코더 5 ms 만큼 줄었는가 (C):** 12 와 같은 측정. 720p30 43 ms 가
   38 안팎이어야 한다.
15. **차 쪽 되먹임을 끊은 것이 실차에서 보이는가 (C): ✅ 확인됨 (2026-09-19, #84) — 드롭 0 · 늦음 0 ·
   키프레임 요청 4(전부 인코더 재시작). 인트라 리프레시는 **기본 꺼짐으로 되돌렸고 저장하지도 않는다**
   (car-tests §16: 벤더가 받아 주는지 미확인 · 켜도 요청 IDR 은 그대로 · 버스트는 이미 사라졌다).
   성적을 재려면 같은 세션에서 껐다 켜고 `keys`·`keyBytes` 를 견준다. (그 전:) ⚠️ 절반만 (2026-09-19, #79, car-tests §14) — 밀린 프레임은
   실제로 안 버린다(`늦음 1266`). 그런데 `드롭 643` 이 남았고 그것은 전부 **다른 고리**였다: 인코더가 다시 서면
   키프레임까지 버리는데(재동기), 그 드롭이 abr 에 혼잡으로 가 비트레이트를 바닥까지 끌어내렸다(질문 19). 인트라
   리프레시는 이 세션에서 안 켰다.** (원래 질문:) 2026-09-18 의 리포트 재검토 결론은 "병목은 인코더 속도가
...
20. **새 APK 를 깐 뒤 차의 페이지가 새 것으로 바뀌는가 (C): ✅ 확인됨 (2026-09-19, #84) — 리포트에
   `stats.webBuild = d8fd1d2` 가 폰과 같고 요약에 `⚠페이지 build=` 가 없다.** (원래 질문:) #83 은 폰만 새 빌드였고 차의 탭은 39 분 전에
   연 옛 페이지였다 — 서버가 다시 서도 소켓이 스스로 이어지므로 탭은 그대로 돈다(car-tests §15). 이 빌드부터
   페이지가 자기 빌드를 알고(`stats.webBuild`), 다르면 빌드당 한 번 스스로 다시 연다. 볼 것: APK 를 깐 뒤
   차 화면이 한 번 깜빡이고 돌아오는지, 리포트 요약에 `⚠페이지 build=` 가 **없는지**(있으면 그 세션의 차 쪽
   수치는 옛 코드의 것이다). A 로 고정(session-report.spec).
19. **abr 이 재동기를 혼잡으로 읽던 되먹임이 끊겼는가 (C): ✅ 확인됨 (2026-09-19, #84, car-tests §16) —
   1080p60 4.3 분에 **드롭 0, 늦음 0, 적체 초과 0, `decode stall` 0**, 키프레임 요청 4 번(전부 인코더 재시작
   직후), abr `↓1↑0` 이고 그 한 번은 진짜 rtt 101 ms 였다. 실제로 받은 비트가 2.5~4.9 → **9~10.4 Mbps**.
   되올리기(`↑`)가 도는 것은 더 긴 세션에서 볼 일로 남았다.** (원래 질문:) #79 에서 abr 은 2.5 분에 **자르기 12·올리기 0**,
   인코더가 다시 설 때마다(프리셋·I-QP 변경) 20 초 안에 바닥(공칭의 40% = 4.8 Mbps)이었다. 링크는 멀쩡했다
   (rtt 12~15 ms, 인코딩 p50 7.2 ms, 서버 `videoDropped 0`). 고친 것은 먹이다 — 재동기 중의 표본은 건너뛰고,
   혼잡은 버린 장수 대신 "적체가 한계를 넘은 횟수"로, 재동기 직후의 "늦음" 버스트는 안 센다(`abr.ts`
   `CongestionReader`, e2e `quality.spec`). 같이 고친 것: 키프레임 재요청 백오프 4 초 → 1 초, 재동기 때
   `inFlight` 비우기(가짜 `lag 585ms` 와 자가 되먹임 덫), "키프레임 대기"를 decode stall 로 부르지 않기,
   가운데 밝기 읽기를 지연 측정 중에만. 볼 것: 같은 2 분에서 `비트↓N↑M` 의 M 이 0 이 아니고 N 이 프리셋을
   바꿀 때마다 바닥까지 가지 않는지, `드롭`·`키프레임요청`이 함께 줄었는지, 그리고 그때 1080p60 이 몇 fps
   인지(#79 의 720p60 마지막 15 초는 53fps·`늦음 0`·lag 1~2 ms 였다). ⚠ #83 은 이 질문에 답하지 못했다 —
   차의 페이지가 옛 빌드였다(질문 20). 절차: testing-guide §C 4b.
