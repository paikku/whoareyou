...
핫스팟 요청이 서버를 죽이지 않는다"까지다. 다만 **권한은 여기서도 답이 나왔다**: AOSP(API 36)는 두 시도 모두
`NO_CHANGE_TETHERING_PERMISSION(14)` 으로 거부한다(run #41·#42). 그때 `permissionDenied` 가 서고, 앱은
"설정에서 직접"으로 안내한다. **One UI 도 같은지는 실기기에서만 알 수 있다** — 그것이 이 기능의 갈림길이고
verification-log 의 열린 질문 0번이다. 거부되더라도 서버·VPN 일괄 동작과 위젯은 그대로 쓸모가 있다.
